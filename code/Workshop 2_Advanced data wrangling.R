...
#pivot the data to a tidy format using two functions: pivot_longer() and pivot_wider()
#pivot_longer() is used to transform data from a wide format to a long format,
#pivot_wider() is used to transform data from a long format to a wide format.
tidyr:pivot_longer()
tidyr:pivot_wider()
...
#cols: specifies the columns to pivot, in this case, all columns that start with "wk".
#names_to: specifies the name of the new column that will contain the names of the original columns, in this case, "week".
#values_to: specifies the name of the new column
# that will contain the values from the original columns, in this case, "rank".
...
#after
#we want to tidy the data by pivoting the bp1 and bp2 columns into a longer format
df |>
  pivot_longer(
    cols =bp1:bp2,
...
#We’ll use an example from R4DS to explore pivot_wider() 
#looking at the cms_patient_experience dataset from the Centers of Medicare and Medicaid
